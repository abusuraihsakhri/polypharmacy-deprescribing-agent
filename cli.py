"""
Command Line Interface for Polypharmacy Deprescribing Agent.
"""
import argparse
import csv
import json
...
# Set default audit key for CLI usage if not already configured
os.environ.setdefault("AUDIT_SECRET_KEY", "polypharmacy-deprescribing-agent-cli-default-key")
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="polypharmacy-deprescribing-agent", description="Polypharmacy Deprescribing Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  POLYPHARMACY DEPRESCRIBING AGENT")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Polypharmacy Deprescribing Agent Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "serve":
        ...
        print(f"Starting Polypharmacy Deprescribing Agent API server on http://{args.host}:{args.port}")
